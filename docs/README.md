...
| expo-sqlite 스키마(Site·Account) | ❌ | CLAUDE.md §5 데이터 구조 |
| Home(즐겨찾기 영역 + 전체 목록 + 계정 수 배지) | ❌ | |
| 사이트 추가/수정/삭제 (URL만 필수 · 이름 자동 제안) | ❌ | [`SITE_SYSTEM.md`](./SITE_SYSTEM.md) §2 |
| 사이트 아이콘 | ❌ | **직접 fetch + 로컬 캐시 + 이니셜 폴백** 확정(2026-08-14). 외부 서비스 🚫 |
| 사이트 상세 + Open in Browser | ❌ | 외부 기본 브라우저. WebView 금지 |
| 계정 CRUD (사이트당 N개 · 전 필드 선택) | ❌ | |
| 민감한 메모(전체 숨김 + 눈 아이콘) | ❌ | 숨김 ≠ 암호화 표기 규칙(CLAUDE.md §8) |
| 검색(이름·URL·계정 이름·ID·메모) | ❌ | **입구는 홈 검색바 하나**(2026-08-14) — 네비는 4탭(Home·+·Favorites·Settings) |
| 즐겨찾기(등록/해제 + 탭) | ❌ | |
| 하단 네비 4탭 골격(+는 site-add 모달 동작) | ✅ | 2026-08-14 Phase 0 — 화면은 전부 골격, `t()` 키로 작성 |
...
# npm run check:i18n           # ⏭ i18n 착수 시 조각 스크립트 이식
```
