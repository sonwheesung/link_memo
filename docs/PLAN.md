...
| 2 | DB + 사이트·계정 CRUD (핵심 도메인) | ✅ 2026-08-14 — typecheck·lint·콜드 번들(1555모듈) 통과. ⏳ 실기기 CRUD 왕복 확인(사용자) |
| 3 | 검색 · 즐겨찾기 · 사이트 아이콘 | ✅ 2026-08-14 — typecheck·lint·check:i18n(50키)·번들 통과. ⏳ 실기기 확인(사용자) |
| 4 | i18n 완성 — 5개 언어 + 검사 스크립트 | ✅ 2026-08-14 — `check:i18n` OK(5언어·47키). Phase 2와 함께 진행(사용자 지시). ⚠ ja·zh는 원어민 검수 전. 언어 수동 변경 화면 포함 |
| 5 | common_server 연동 — 공지·점검 게이트·문의 | ✅ 2026-08-14 — `linkmemo` 등록 + **bootstrap 200 실측** + SDK 복사 + 게이트·공지·문의 화면. **+기기 subject 문의 귀속·내역/답변/상태 화면**(사용자 요구로 서버 `POST /v1/devices` 신설·배포·E2E 실측). ⏸ 디스코드 웹훅 env(사용자 웹훅 URL 필요) · 소프트 업데이트 안내(latest) 미구현 |
| 6 | 광고 — dev build 전환 · 배너 · App Open · UMP | ✅ 코드 2026-08-14(typecheck·lint·i18n·**릴리스 AAB 빌드 성공 13m50s**). ⏳ 실기기 광고 노출 확인은 테스트 빌드 설치 후 |
| 6.5 | **비공개 테스트 개시**(2026-08-14 사용자 결정 — 14일 시계 병행) | 🔨 — Play 콘솔 앱 생성 ✅ · Alpha 트랙(국가 177·테스터) ✅ · **AAB 업로드 ✅(사용자, 버전 1/API 24+/22.5MB)** · **앱 콘텐츠 선언 10종 전부 ✅**(2026-08-14 브라우저 대행: 처리방침 URL·광고·로그인 세부·금융·건강·광고 ID·타겟층 13+·정부·**콘텐츠 등급 IARC(ESRB E·PEGI 3)**·**데이터 보안**(대략 위치·앱 상호작용·기기 ID=광고 수집/공유, 문의 본문=선택 수집)) · 스토어 등록정보 **텍스트 초안 ✅**(§38 영문). **남음: ① 그래픽 자산(앱 아이콘 512·피처 그래픽 1024×500·폰 스크린샷 2+) ② 테스터 12명 채우기 ③ 검토 전송.** ⚠ 심사 전 처리방침을 `vivace-games.com/linkmemo/privacy`에 실게시 필수 |
...
- 검색: **입구는 홈 우상단 🔍 하나.** 대상 5필드(사이트 이름·URL·계정 이름·ID·메모), 디바운스,
  민감 메모는 매치돼도 미리보기 마스킹(SITE_SYSTEM §5)
- 즐겨찾기: 토글 + **홈 상단 섹션**(전용 탭 없음 — 2026-08-14 단일 화면 결정)
