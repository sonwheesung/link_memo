...
| 9 | 즐겨찾기 | 로컬 | 사이트 단위 등록/해제. **홈 상단 섹션만**(전용 탭 없음 — 2026-08-14 단일 화면 결정) |
...
하단 네비게이션: ~~5탭~~ → ~~4탭~~ → ~~3탭~~ → **없음 — 단일 화면 구조**(2026-08-14 사용자 결정).
홈이 유일한 메인 화면이고, **즐겨찾기는 홈 상단 섹션**으로 들어간다(전용 탭·전용 화면 없음).
입구는 전부 **홈 우상단 헤더 버튼 3개**: [🔍 검색][＋ 추가][⚙ 설정] — 각각 스택 화면으로 이동.
...
- ~~문의하기는 v1 익명 단방향 — 답변 확인 화면 없음~~ → **정정(2026-08-14 사용자 요구: 문의 목록·답변·
  상태가 보여야 한다): 기기 토큰(device subject) 방식으로 문의를 귀속한다.** 앱이 최초 1회 UUID를 만들어
  SecureStore에 보관하고, common_server `POST /api/v1/devices`로 익명 subject + 서명 토큰을 발급받는다.
  이후 문의는 그 기기에 귀속 → `tickets/mine`으로 목록·답변·상태 조회. **로그인·계정이 아니다** —
  이메일도 이름도 없고, 앱을 지우면 연결이 끊긴다(그 한계를 화면에 고지). common_server PLAN §2의
  "비회원 앱 device subject" 설계를 구현한 것.
...
- 계정 메모에 **숨김 플래그** 하나. ~~카드에서 `••••` + 눈 토글~~ → **카드에는 `🔒 숨겨진 메모` 상태
  라벨만, 내용 확인은 계정 상세 화면의 눈 아이콘으로**(2026-08-14 사용자 피드백 — SITE_SYSTEM §3).
...
| 4 | **단일 화면 — 네비게이션 없음** (사용자 결정 · 2026-08-14 세 번 재정정 끝 확정) | ~~5탭~~→~~4탭~~→~~3탭~~→제거. 홈 우상단 [🔍][＋][⚙] 3버튼이 전 입구. 즐겨찾기는 홈 섹션. 빠르고 가벼운 유틸리티라는 정체성(기둥 4)에 가장 정합 |
...
| 6 | **출시 계정 = Vivace Games Studio(개인, `6329667596149711059`)** (2026-08-14) | 정본 `volleyball/docs/GOOGLE_ACCOUNT_CASE.md` — 2026-08-14 완전 활성화, 앞으로의 계정. 개인 계정 → 비공개 테스트 12명×14일 필요 |
| 7 | **비공개 테스트 즉시 시작** (2026-08-14 사용자 결정) | "2주 안에 개발 완료 예상" — 14일 시계를 개발과 병행. Play 콘솔 앱 생성 + closed track + AAB 업로드 |
| 8 | **패키지명 = `com.vivacegames.linkmemo`** (2026-08-14 — 영구, 변경 불가) | 브랜드(Vivace Games) 도메인 역순. 형제 앱들의 잡다한 패턴(com.son0925.* 등)보다 브랜드 통일 |
| 9 | **서명 = 신규 업로드 키스토어 + Play App Signing** (2026-08-14) | `credentials/linkmemo-upload.jks`(커밋 금지) + `C:\private_key` 백업(배구 규약 승계) |
