...
| 8 | 검색 | 로컬 | 사이트 이름 · URL · 계정 이름 · ID · 메모 전체 대상. **입구는 홈 검색바 하나뿐**(2026-08-14 사용자 결정) |
...
하단 네비게이션: ~~`Home · Search · + · Favorites · Settings`~~ → **`Home · + · Favorites · Settings` 4탭**
(2026-08-14 정정 — 검색 입구를 홈 검색바 하나로 통일하며 Search 탭 제거. 조각이 같은 강등을 실증했다).
...
| 4 | **검색 입구 1개 = 홈 검색바** (사용자 결정) | Search 탭·우상단 돋보기 제거 → 네비 4탭. 조각의 검색 탭 강등(2026-08-12) 실증 승계 |
